...
// ========================================================================
    {
        ...
        //$Posts["client"]["PHP_AUTH_DIGEST"] = $_SERVER["PHP_AUTH_DIGEST"];
        //$Posts["client"]["PHP_AUTH_USER"] = $_SERVER["PHP_AUTH_USER"];
        //$Posts["client"]["PHP_AUTH_PW"] = $_SERVER["PHP_AUTH_PW"];
        //$Posts["client"]["AUTH_TYPE"] = $_SERVER["AUTH_TYPE"];
...
// ========================================================================
    public function actionGet($token=null,$type=send,$seq=null,$pid=null,$an=null,$cid=null,$prov=null,$lastupdate=null,$date=null)
// ========================================================================
    {
        $headers = Yii::$app->response->headers;
        $headers->add('Content-Type', 'application/json');
...
// ========================================================================
    private function GetBySeq($token,$type,$seq)
// ========================================================================
    {
        /*"prov": "40",
          "region": "07",
        ...
        $Sql = 'SELECT r.* FROM refer_history r LEFT JOIN chospital h on r.HOSPCODE=h.hoscode '
                . ' WHERE SEQ="'.$seq.'" AND h.provcode="'.$getToken["prov"].'" ';
    ...
    // ========================================================================
        private function GetByDate($token,$type,$date)
    // ========================================================================
        {
            /*"prov": "40",
              "region": "07",
            ...
            if (strtolower($type)=='send'){
                $Sql = 'SELECT r.* '
                        . ' FROM refer_history r LEFT JOIN chospital h on r.HOSPCODE=h.hoscode '
                        . ' WHERE substr(r.DATETIME_REFER,1,10)="'.$date.'" '
                        . ' AND h.provcode="'.$getToken["prov"].'" ';
            } else {
                $Sql = 'SELECT r.* '
                        . ' FROM refer_history r LEFT JOIN chospital d on r.HOSP_DESTINATION=d.hoscode '
                        . ' LEFT JOIN chospital h on r.HOSPCODE=h.hoscode '
                        . ' WHERE substr(r.DATETIME_REFER,1,10)="'.$date.'" AND r.HOSP_DESTINATION!="" '
                        . ' AND d.provcode="'.$getToken["prov"].'" AND  h.provcode!="'.$getToken["prov"].'" ';
            }
            $referHistory= Yii::$app->db_nrefer_hdc->createCommand($Sql)->queryAll();
            $pid = $referHistory[0]["PID"];
            $hospcode = $referHistory[0]["HOSPCODE"];
